...
6. Run the SDK launcher once to create `~/.companion-core-lite/identity/PERSONA.md` and the visible `sdk-config.json` (`%USERPROFILE%\.companion-core-lite` on Windows); it deliberately stops while the persona is still a scaffold. Invite the companion to replace that scaffold with their own compact identity, voice, relationships, boundaries, and participation preferences. Do not silently write a character card over them. Put optional, provenance-aware narrative slices in `CONTINUITY.md`.
...
9. Give the human the exact absolute launcher command for their OS: `scripts/start-sdk-runtime.ps1` in PowerShell on Windows, or `bash scripts/start-sdk-runtime.sh` on macOS. They run it in a private local terminal and enter the Discord token only into its masked prompt. The launcher installs pinned dependencies locally if needed. If the normal Codex login lives only in macOS Keychain, the launcher opens one visible browser login and stores that login only in the isolated runtime home; it never asks for an API key or access token. Keep that terminal open while presence is wanted; Ctrl+C stops it.
...
If a granted turn fails, inspect `~/.companion-core-lite/sdk-failed-turn.json` (`%USERPROFILE%\.companion-core-lite\sdk-failed-turn.json` on Windows); the runtime deliberately will not auto-retry it because a post may already have succeeded. Never replay it without checking Discord first.
